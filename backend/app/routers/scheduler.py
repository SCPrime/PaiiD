...
async def list_schedules(_=Depends(require_current_user)):
...
async def create_schedule(schedule_data: ScheduleCreate, _=Depends(require_current_user)):
...
async def update_schedule(
    schedule_id: str, schedule_data: ScheduleUpdate, _=Depends(require_current_user)
):
...
async def delete_schedule(schedule_id: str, _=Depends(require_current_user)):
    """Delete a schedule"""
    schedule = _load_schedule(schedule_id)
...
async def pause_all_schedules(_=Depends(require_current_user)):
...
async def resume_all_schedules(_=Depends(require_current_user)):
...
async def list_executions(
    limit: int = 20, schedule_id: str | None = None, _=Depends(require_current_user)
):
...
async def list_pending_approvals(_=Depends(require_current_user)):
...
async def approve_trade(approval_id: str, _=Depends(require_current_user)):
...
async def reject_trade(approval_id: str, decision: ApprovalDecision, _=Depends(require_current_user)):
...
@router.get("/status")
async def scheduler_status(_=Depends(require_current_user)):
