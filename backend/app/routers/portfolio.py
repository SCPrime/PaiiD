...
@router.get("/account")
def get_account(_: object = Depends(get_current_user)):
...
def get_positions(_: object = Depends(get_current_user), cache: CacheService = Depends(get_cache)):
...
def get_position(symbol: str, _: object = Depends(get_current_user)):
