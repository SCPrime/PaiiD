...
async def detect_patterns(
    ...
    try:
        ...
        if ohlcv_data.empty:
            raise HTTPException(
                status_code=404, detail=f"No data available for symbol {request.symbol}"
            )
        ...
        confidence_summary = {
            ...
            "moderate": len(
                [p for p in filtered_patterns if 0.6 <= p.confidence < 0.7]
            ),
...
async def get_comprehensive_insights(
    request: MLInsightsRequest, current_user: User = Depends(get_current_user_unified)
):
    """
    ...
    try:
            ...
            try:
                ...
                patterns_result = {
                    ...
                    "confidence_distribution": {
                        ...
                        "strong": len(
                            [p for p in patterns if 0.7 <= p.confidence < 0.8]
                        ),
                        "moderate": len(
                            [p for p in patterns if 0.6 <= p.confidence < 0.7]
                        ),
                    },
                }
            except Exception as e:
        ...
        if patterns_result:
            pattern_confidence = (
                sum(p["confidence"] for p in patterns_result["top_patterns"])
                / len(patterns_result["top_patterns"])
                if patterns_result["top_patterns"]
                else 0.5
            )
        ...
        if patterns_result and patterns_result["top_patterns"]:
            ...
            key_insights.append(
                f"Strong {top_pattern['pattern_type']} pattern detected with {top_pattern['confidence']:.1%} confidence"
            )

        if regime_result:
            key_insights.append(
                f"Market in {regime_result['current_regime']} regime with {regime_result['trend_direction']} trend"
            )
            key_insights.extend(regime_result["trading_implications"][:2])

        if sentiment_result:
            key_insights.append(
                f"Overall sentiment: {sentiment_result['overall_sentiment']}"
            )
        ...
        "pattern_types": [
            {
                ...
                "category": "reversal"
                if "top" in pattern_type.value
                or "bottom" in pattern_type.value
                or "shoulder" in pattern_type.value
                else "continuation"
                if "triangle" in pattern_type.value
                or "flag" in pattern_type.value
                or "pennant" in pattern_type.value
                else "candlestick"
                if pattern_type.value
                in ["hammer", "doji", "engulfing", "morning_star", "evening_star"]
                else "volume"
                if "volume" in pattern_type.value
...
async def ml_health_check():
    ...
    try:
        ...
        try:
            ...
            pattern_status = f"unhealthy: {e!s}"
        ...
        try:
            ...
            regime_status = f"unhealthy: {e!s}"

        return {
            "status": "healthy"
            if pattern_status == "healthy" and regime_status == "healthy"
            else "degraded",
            "services": {
