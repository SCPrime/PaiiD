...
async def stream_positions(_=Depends(require_current_user), cache: CacheService = Depends(get_cache)):
    """
...
async def stream_market_indices(
    _=Depends(require_current_user), cache: CacheService = Depends(get_cache)
):
    """
...
async def stream_status(_=Depends(require_current_user)):
    """
