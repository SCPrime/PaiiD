...
async def execute(request: Request, req: ExecRequest, _: object = Depends(get_current_user)):
    """
...
def kill(state: bool, _: object = Depends(get_current_user)):
    set_kill(state)
...
def create_order_template(
    template: OrderTemplateCreate, db: Session = Depends(get_db), _: object = Depends(get_current_user)
):
...
def list_order_templates(
    skip: int = 0, limit: int = 100, db: Session = Depends(get_db), _: object = Depends(get_current_user)
):
...
def get_order_template(template_id: int, db: Session = Depends(get_db), _: object = Depends(get_current_user)):
...
def delete_order_template(
    template_id: int, db: Session = Depends(get_db), _: object = Depends(get_current_user)
):
...
def use_order_template(template_id: int, db: Session = Depends(get_db), _: object = Depends(get_current_user)):
