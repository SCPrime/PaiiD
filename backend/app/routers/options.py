...
class ExpirationDate(BaseModel):
    """Available option expiration date"""
...
@router.get(
    "/expirations/{symbol}",
    response_model=list[ExpirationDate],
    dependencies=[Depends(require_bearer)],
)
async def get_option_expirations(symbol: str) -> list[ExpirationDate]:
    """Return available option expiration dates for the requested symbol."""

    symbol = symbol.upper()
    api_key = settings.TRADIER_API_KEY
    base_url = settings.TRADIER_API_BASE_URL
    ...
    try:
        ...
        raise HTTPException(status_code=status_code, detail=f"Tradier API error: {detail}") from exc
    except requests.exceptions.RequestException as exc:
        raise HTTPException(
            status_code=502,
            detail=f"Unable to reach Tradier API: {exc}",
        ) from exc

    try:
        payload = response.json()
    except ValueError as exc:
        raise HTTPException(status_code=502, detail="Tradier API returned invalid JSON") from exc
    ...
    if isinstance(raw_dates, str):
        ...
        raw_dates = [value for value in raw_dates.values() if value]
    ...
    for value in raw_dates:
        ...
        expirations.append(ExpirationDate(date=date_str, days_to_expiry=days_to_expiry))

    expirations.sort(key=lambda entry: entry.date)
    return expirations
