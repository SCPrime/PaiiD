...
async def get_quote(
    symbol: str, _=Depends(get_current_user), cache: CacheService = Depends(get_cache)
):
...
async def get_quotes(symbols: str, _=Depends(get_current_user)):
...
async def get_bars(
    symbol: str, timeframe: str = "daily", limit: int = 100, _=Depends(get_current_user)
):
...
async def scan_under_4(_=Depends(get_current_user)):
