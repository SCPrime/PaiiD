...
async def get_quote(
    symbol: str, _: object = Depends(get_current_user), cache: CacheService = Depends(get_cache)
):
...
async def get_quotes(symbols: str, _: object = Depends(get_current_user)):
...
async def get_bars(
    symbol: str, timeframe: str = "daily", limit: int = 100, _: object = Depends(get_current_user)
):
...
async def scan_under_4(_: object = Depends(get_current_user)):
