...
class AdvancedRegimeDetector:
    ...
    def detect_regime(self, 
                     ...
                     market_data: pd.DataFrame = None) -> RegimeAnalysis:
        """
        ...
        try:
            ...
            # Combine results
            final_regime = self._combine_regime_results(regime_ml, regime_technical, regime_volatility)
            ...
            logger.info(f"Detected regime: {analysis.current_regime.value} "
    ...
    def _calculate_features(self, 
        ...
        try:
            ...
            features['breakout_potential'] = self._calculate_breakout_potential(close, high, low)
            features['reversal_potential'] = self._calculate_reversal_potential(close)
            
            # Market structure features
            features['support_resistance_strength'] = self._calculate_sr_strength(high, low)
    ...
    def _detect_regime_ml(self, features: Dict[str, float]) -> Tuple[MarketRegime, float]:
    ...
    def _detect_regime_technical(self, ohlcv_data: pd.DataFrame, features: Dict[str, float]) -> Tuple[MarketRegime, float]:
    ...
    def _detect_regime_volatility(self, ohlcv_data: pd.DataFrame, features: Dict[str, float]) -> Tuple[MarketRegime, float]:
    ...
    def _combine_regime_results(self, 
                               ...
                               regime_volatility: Tuple[MarketRegime, float]) -> MarketRegime:
    ...
    def _generate_regime_analysis(self, 
                                 ...
                                 ohlcv_data: pd.DataFrame) -> RegimeAnalysis:
    ...
    def _calculate_trend_direction(self, close: np.ndarray) -> float:
        ...
        if len(sma_short) > 0 and len(sma_long) > 0 and not np.isnan(sma_short[-1]) and not np.isnan(sma_long[-1]):
    ...
    def _calculate_macd_signal(self, close: np.ndarray) -> float:
        ...
        if len(macd) > 0 and len(signal) > 0 and not np.isnan(macd[-1]) and not np.isnan(signal[-1]):
    ...
    def _calculate_price_volume_correlation(self, close: np.ndarray, volume: pd.Series) -> float:
    ...
    def _calculate_breakout_potential(self, close: np.ndarray, high: np.ndarray, low: np.ndarray) -> float:
    ...
    def _calculate_market_features(self, market_data: pd.DataFrame) -> Dict[str, float]:
        ...
        if 'vix' in market_data.columns:
            ...
            features['vix_trend'] = float(np.mean(np.diff(vix[-5:]))) if len(vix) > 5 else 0.0
        
        # Sector features
        if 'sector_performance' in market_data.columns:
            sector_perf = market_data['sector_performance'].values
            features['sector_dispersion'] = float(np.std(sector_perf)) if len(sector_perf) > 0 else 0.0
    ...
    def _get_trend_direction(self, regime: MarketRegime, features: Dict[str, float]) -> str:
    ...
    def _get_trading_implications(self, regime: MarketRegime, features: Dict[str, float]) -> List[str]:
    ...
    def _calculate_risk_level(self, regime: MarketRegime, features: Dict[str, float]) -> str:
        """Calculate risk level based on regime and features"""
        volatility = features.get('volatility', 0.02)
        
        if regime == MarketRegime.HIGH_VOLATILITY or volatility > 0.05:
            return "extreme"
        elif regime in [MarketRegime.TRENDING_BEAR, MarketRegime.REVERSAL] or volatility > 0.03:
            return "high"
        elif regime in [MarketRegime.TRENDING_BULL, MarketRegime.BREAKOUT] or volatility > 0.02:
            return "medium"
        else:
            return "low"
    
    def _get_recommended_strategies(self, regime: MarketRegime, features: Dict[str, float]) -> List[str]:
        """Get recommended strategies for regime"""
        strategies = []
        
        if regime == MarketRegime.TRENDING_BULL:
            strategies.extend(["Momentum trading", "Trend following", "Breakout strategies"])
        elif regime == MarketRegime.TRENDING_BEAR:
            strategies.extend(["Short selling", "Contrarian strategies", "Defensive positioning"])
        elif regime == MarketRegime.RANGING:
            strategies.extend(["Mean reversion", "Range trading", "Options strategies"])
        elif regime == MarketRegime.HIGH_VOLATILITY:
            strategies.extend(["Volatility trading", "Options strategies", "Risk management"])
        elif regime == MarketRegime.BREAKOUT:
            strategies.extend(["Breakout trading", "Momentum strategies", "Volume analysis"])
        
        return strategies
    
    def _calculate_heuristic_probabilities(self, features: Dict[str, float]) -> Dict[MarketRegime, float]:
    ...
    def _get_default_regime_analysis(self) -> RegimeAnalysis:
        ...
        return RegimeAnalysis(
            ...
            momentum_score=0.0,
            regime_duration=10,
            regime_probability={regime: 0.1 for regime in MarketRegime},
            key_indicators={},
