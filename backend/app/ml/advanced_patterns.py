...
class AdvancedPatternDetector:
    ...
    def detect_patterns(self, 
                       ...
                       volume_data: pd.Series = None) -> List[PatternSignal]:
        """
        ...
        try:
            ...
            patterns.extend(self._detect_support_resistance_patterns(ohlcv_data, indicators))
            
            # Filter by confidence and sort
            patterns = [p for p in patterns if p.confidence >= self.min_confidence]
            patterns.sort(key=lambda x: x.confidence, reverse=True)
            
            logger.info(f"Detected {len(patterns)} patterns with confidence >= {self.min_confidence}")
    ...
    def _detect_reversal_patterns(self, data: pd.DataFrame, indicators: Dict) -> List[PatternSignal]:
        """Detect reversal patterns"""
        patterns = []
    ...
    def _detect_head_and_shoulders(self, data: pd.DataFrame, indicators: Dict) -> List[PatternSignal]:
            ...
            if len(peaks) >= 3:
                ...
                for i in range(len(peaks) - 2):
                    ...
                    if (high[head] > high[left_shoulder] and 
                        ...
                        confidence = self._calculate_hs_confidence(data, left_shoulder, head, right_shoulder)
                        ...
                        if confidence >= self.min_confidence:
                            patterns.append(PatternSignal(
                                ...
                                direction="bearish",
                                target_price=high[left_shoulder] - (high[head] - high[left_shoulder]),
                                stop_loss=high[head] * 1.02,
                                risk_reward_ratio=self._calculate_risk_reward(
                                    high[head], high[left_shoulder], 
                                    high[left_shoulder] - (high[head] - high[left_shoulder])
                                ),
                                timeframe="medium",
                                volume_confirmation=self._check_volume_confirmation(data, head),
                                trend_alignment=self._check_trend_alignment(indicators, "bearish"),
                                key_levels=[high[left_shoulder], high[head], high[right_shoulder]],
    ...
    def _detect_double_tops_bottoms(self, data: pd.DataFrame, indicators: Dict) -> List[PatternSignal]:
        ...
        try:
            ...
            for i in range(len(peaks) - 1):
                ...
                if (abs(high[peak1] - high[peak2]) / high[peak1] < 0.02 and
                    peak2 - peak1 > 5):  # Minimum distance between peaks
                    
                    confidence = self._calculate_double_pattern_confidence(data, peak1, peak2, "top")
                    ...
                    if confidence >= self.min_confidence:
                        patterns.append(PatternSignal(
                            ...
                            strength=self._get_strength(confidence),
                            direction="bearish",
                            target_price=high[peak1] - (high[peak1] - min(low[peak1:peak2])),
                            ...
                            timeframe="medium",
                            volume_confirmation=self._check_volume_confirmation(data, peak2),
                            trend_alignment=self._check_trend_alignment(indicators, "bearish"),
            ...
            for i in range(len(troughs) - 1):
                ...
                if (abs(low[trough1] - low[trough2]) / low[trough1] < 0.02 and
                    trough2 - trough1 > 5):  # Minimum distance between troughs
                    
                    confidence = self._calculate_double_pattern_confidence(data, trough1, trough2, "bottom")
                    ...
                    if confidence >= self.min_confidence:
                        patterns.append(PatternSignal(
                            ...
                            strength=self._get_strength(confidence),
                            direction="bullish",
                            target_price=low[trough1] + (max(high[trough1:trough2]) - low[trough1]),
                            stop_loss=low[trough1] * 0.99,
                            risk_reward_ratio=self._calculate_risk_reward(
                                low[trough1], max(high[trough1:trough2]),
                                low[trough1] + (max(high[trough1:trough2]) - low[trough1])
                            ),
                            timeframe="medium",
                            volume_confirmation=self._check_volume_confirmation(data, trough2),
                            trend_alignment=self._check_trend_alignment(indicators, "bullish"),
    ...
    def _detect_continuation_patterns(self, data: pd.DataFrame, indicators: Dict) -> List[PatternSignal]:
        """Detect continuation patterns"""
        patterns = []
    ...
    def _detect_triangle_patterns(self, data: pd.DataFrame, indicators: Dict) -> List[PatternSignal]:
                ...
                if self._is_ascending_triangle(recent_highs, recent_lows):
                    ...
                    if confidence >= self.min_confidence:
                        patterns.append(PatternSignal(
                            ...
                            risk_reward_ratio=self._calculate_risk_reward(
                                close[-1], min(recent_lows), max(recent_highs) * 1.05
                            ),
                            timeframe="short",
                            volume_confirmation=self._check_volume_confirmation(data, -1),
                            trend_alignment=self._check_trend_alignment(indicators, "bullish"),
                ...
                if self._is_descending_triangle(recent_highs, recent_lows):
                    ...
                    if confidence >= self.min_confidence:
                        patterns.append(PatternSignal(
                            ...
                            risk_reward_ratio=self._calculate_risk_reward(
                                close[-1], max(recent_highs), min(recent_lows) * 0.95
                            ),
                            timeframe="short",
                            volume_confirmation=self._check_volume_confirmation(data, -1),
                            trend_alignment=self._check_trend_alignment(indicators, "bearish"),
    ...
    def _detect_candlestick_patterns(self, data: pd.DataFrame) -> List[PatternSignal]:
        ...
        try:
            ...
            if self._is_hammer_pattern(recent_data):
                confidence = self._calculate_candlestick_confidence(recent_data, "hammer")
    ...
    def _detect_volume_patterns(self, data: pd.DataFrame, volume_data: pd.Series) -> List[PatternSignal]:
    ...
    def _detect_support_resistance_patterns(self, data: pd.DataFrame, indicators: Dict) -> List[PatternSignal]:
        ...
        try:
            ...
            for support in support_levels:
                if current_price < support * 0.99:  # 1% below support
                    confidence = self._calculate_break_confidence(data, support, "support")
            ...
            for resistance in resistance_levels:
                if current_price > resistance * 1.01:  # 1% above resistance
                    confidence = self._calculate_break_confidence(data, resistance, "resistance")
    ...
    def _calculate_hs_confidence(self, data: pd.DataFrame, left_shoulder: int, head: int, right_shoulder: int) -> float:
        """Calculate Head and Shoulders confidence"""
        # Simplified confidence calculation
        return 0.7  # Placeholder
    
    def _calculate_double_pattern_confidence(self, data: pd.DataFrame, point1: int, point2: int, pattern_type: str) -> float:
        """Calculate double pattern confidence"""
        # Simplified confidence calculation
        return 0.6  # Placeholder
    
    def _calculate_triangle_confidence(self, data: pd.DataFrame, triangle_type: str) -> float:
        """Calculate triangle pattern confidence"""
        # Simplified confidence calculation
        return 0.65  # Placeholder
    
    def _calculate_candlestick_confidence(self, data: pd.DataFrame, pattern_type: str) -> float:
        """Calculate candlestick pattern confidence"""
        # Simplified confidence calculation
        return 0.6  # Placeholder
    
    def _calculate_break_confidence(self, data: pd.DataFrame, level: float, break_type: str) -> float:
