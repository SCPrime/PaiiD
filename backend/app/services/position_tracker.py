...
class PositionTrackerService:
    ...
    async def close_position(self, position_id: str, limit_price: Optional[float] = None) -> Dict:
    ...
    def _parse_expiration(self, option_symbol: str) -> str:
        ...
        date_part = option_symbol[option_symbol.index(next(c for c in option_symbol if c.isdigit())):option_symbol.index(next(c for c in option_symbol if c in 'CP'))]
    ...
    def _parse_option_symbol(self, option_symbol: str):
        ...
        # Extract strike
        strike_part = option_symbol[option_symbol.index(next(c for c in option_symbol if c in 'CP'))+1:]
