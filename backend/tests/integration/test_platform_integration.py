...
class IntegrationTestSuite:
    ...
    def _test_token_validation(self):
        ...
        response = self.session.get(
            f"{self.base_url}/api/auth/session", headers=headers
        )
    ...
    def _test_logout(self):
        ...
        headers = {"Authorization": f"Bearer {self.auth_token}"}
        response = self.session.post(
            f"{self.base_url}/api/auth/logout", headers=headers
        )
    ...
    def _test_error_handling(self):
        ...
        # Test 400 error
        response = self.session.post(
            f"{self.base_url}/api/auth/login", json={"invalid": "data"}
        )
    ...
    def _test_position_management(self):
        ...
        response = self.session.post(
            ...
            headers=headers,
            json={"symbol": "AAPL", "quantity": 100, "cost_basis": 150.00},
        )
    ...
    def _test_portfolio_sync(self):
        ...
        headers = {"Authorization": f"Bearer {self.auth_token}"}
        response = self.session.post(
            f"{self.base_url}/api/portfolio/sync", headers=headers
        )
    ...
    def _test_order_history(self):
        ...
        headers = {"Authorization": f"Bearer {self.auth_token}"}
        response = self.session.get(
            f"{self.base_url}/api/orders/history", headers=headers
        )
    ...
    def _test_order_cancellation(self):
        ...
        response = self.session.delete(
            f"{self.base_url}/api/orders/test_order_id", headers=headers
        )
    ...
    def generate_report(self, total_duration: float) -> dict[str, Any]:
        ...
        report = {
            ...
            "performance_metrics": {
                ...
                "average_response_time": round(avg_response_time, 3),
                "fastest_test": min(
                    self.test_results, key=lambda x: x.duration
                ).test_name
            ...
            "environment": {"base_url": self.base_url, "test_suite_version": "1.0.0"},
        }
    ...
    def print_report(self, report: dict[str, Any]):
        ...
        for category, stats in report["categories"].items():
            print(
                f"   {category}: {stats['passed']} passed, {stats['failed']} failed, {stats['skipped']} skipped"
            )
