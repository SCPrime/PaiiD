...
import type { ValidationResult, ValidationError, ValidationWarning } from "./schema";
...
function validateUniverse(
  ...
  if (!universe.filters) {
    ...
    if (filters.price_between) {
      if (!Array.isArray(filters.price_between) || filters.price_between.length !== 2) {
        errors.push({
    ...
    if (filters.max_option_spread !== undefined && filters.max_option_spread < 0) {
      errors.push({
    ...
    if (filters.min_option_oi_per_strike !== undefined && filters.min_option_oi_per_strike < 0) {
      errors.push({
...
function validateEntry(entry: any, errors: ValidationError[], warnings: ValidationWarning[]) {
...
function validatePosition(position: any, errors: ValidationError[], warnings: ValidationWarning[]) {
...
function validateSizing(sizing: any, errors: ValidationError[], _warnings: ValidationWarning[]) {
  ...
  if (
    (sizing.allocation_type === "cash_max_loss" || sizing.allocation_type === "max_loss") &&
...
function validateExits(exits: any, errors: ValidationError[], _warnings: ValidationWarning[]) {
  if (
  ...
  if (exits.max_loss_pct !== undefined && (exits.max_loss_pct < 0 || exits.max_loss_pct > 1)) {
    errors.push({
...
function validateRisk(risk: any, errors: ValidationError[], _warnings: ValidationWarning[]) {
...
function validateBusinessRules(
  ...
  if (
    ...
    const hasNakedShortOption = strategy.position?.legs?.some(
      (leg: any) => (leg.type === "CALL" || leg.type === "PUT") && leg.side === "SELL"
    );

    if (hasNakedShortOption) {
      const hasProtection = strategy.position?.legs?.some(
        (leg: any) => (leg.type === "CALL" || leg.type === "PUT") && leg.side === "BUY"
      );

      if (!hasProtection) {
        errors.push({
          field: "position.legs",
          message: "Sub-$4 strategies with short options must be cash-secured or have defined risk",
  ...
  if (strategy.automation?.execution_mode === "autopilot") {
    const tradingMode =
      typeof window !== "undefined" ? localStorage.getItem("tradingMode") || "paper" : "paper";
    ...
    if (tradingMode === "live") {
      ...
      warnings.push({
        field: "automation.execution_mode",
        message:
          "⚠️ PAPER TRADING: Autopilot enabled for testing. This strategy will execute trades without approval.",
    ...
    if (!hasGates) {
      warnings.push({
        field: "automation",
        message:
          "Consider setting performance thresholds (win rate, Sharpe, max DD) for autopilot mode",
