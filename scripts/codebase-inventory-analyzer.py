...
EXCLUDE_DIRS = {
    ...
    "playwright-report", ".next", "archive"
}
...
class CodebaseAnalyzer:
    ...
    def tag_artifact(self, file_path: Path, reference_count: int, category: str) -> str:
        ...
        if category == "Code":
            ...
            if any(x in name_lower for x in ["main", "index", "app", "start", "deploy", "test"]):
                return "needs verification"
    ...
    def generate_csv(self, output_path: Path):
        ...
        with open(output_path, 'w', newline='', encoding='utf-8') as f:
            ...
            sorted_data = sorted(
                self.files_data,
                key=lambda x: (
                    {"redundant": 0, "needs verification": 1, "current": 2}[x["status"]],
                    x["category"],
                    x["path"]
                )
            )
    ...
    def generate_report(self, output_path: Path):
        ...
        needs_verification = [f for f in self.files_data if f["status"] == "needs verification"]
...
"""
        ...
        redundant_docs = [f for f in redundant_files if f["category"] == "Documentation"]
...
"""
        ...
        verify_docs = [f for f in needs_verification if f["category"] == "Documentation"]
